...
NUM_WOERKER = 4
AUDIO_WIN_SIZE = 1024
LEARNING_RATE =1.284e-4

MIX_PRECISION = 32
USE_SCHEDUERL = True
ACCUMULATION_GRAD_BATCHES = 4
...
pretrained_checkpoint = "checkpoints/00007/checkpoint-epoch=0092-val_l1_loss=0.0997.ckpt"
...
def train(cfg: DictConfig):
    ...
    cfg.ml.num_workers = NUM_WOERKER
    cfg.ml.optim.learning_rate = LEARNING_RATE
    cfg.dataset.win_size = AUDIO_WIN_SIZE
    cfg.dataset.fft_size = AUDIO_WIN_SIZE
    cfg.ml.mix_precision = MIX_PRECISION
    cfg.ml.scheduler.use = USE_SCHEDUERL
    cfg.ml.accumulate_grad_batches = ACCUMULATION_GRAD_BATCHES
    cfg.dataset.aug.volume_aug_rate = VOLUME_AUG_RATE
    ...
    model = AvocodoModule.load_from_checkpoint(checkpoint_path=pretrained_checkpoint, cfg=cfg)
