# copy from https://github.com/ncsoft/avocodo/blob/main/avocodo/models/SBD.py
...
from src.avocodo.utils import get_padding
from src.avocodo.pqmf import PQMF
from src.avocodo.config import SBDConfig
...
class SBD(torch.nn.Module):
    def __init__(self, h:SBDConfig, use_spectral_norm:bool=False):
        super(SBD, self).__init__()
        self.h = h
        self.pqmf = PQMF(*h.pqmf_sbd)
        if True in h.sbd_transpose:
            self.f_pqmf = PQMF(*h.pqmf_fsbd)
        else:
            self.f_pqmf = None
        ...
        for _f, _k, _d, _s, _br, _tr in zip(
            ...
            if _tr:
                segment_dim = h.segment_size // _br[1] - _br[0]
            else:
    ...
    def forward(self, y, y_hat):
        ...
        for d, br, tr in zip(
            self.discriminators, self.h.sbd_band_ranges, self.h.sbd_transpose
        ):
            if tr:
